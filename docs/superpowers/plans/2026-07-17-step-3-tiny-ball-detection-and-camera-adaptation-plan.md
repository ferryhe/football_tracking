...
**Status:** Execution active · PR-T3I complete · PR-T4 held at the feasibility gate
...
- Current engineering PR: none · PR-T3I product and docs-only closeout complete
- Current baseline: `main` after PR #127 closeout
- Current phase: PR-T3I product PR #126 and docs-only closeout PR #127 are merged and cleaned. No engineering PR is active; the next operational step is deployment/refresh plus exact probe reconciliation and evidence collection. PR-T4 remains unauthorized.
...
- Latest integrated validation: the current-diff official backend route passes 2,263 tests with 71 skipped in 628.543 seconds, including OpenAPI `--check`; frontend Vitest passes 1,237/1,237 with 93.18% statements, 90.66% branches, 97.47% functions, and 94.24% lines; all library/application/test TypeScript checks pass; all production builds pass with the mockup sandbox's documented `PORT=3000` and `BASE_PATH=/`; and Chromium passes 44/44, including unseen-check isolation, Konva geometry, and accessibility. The changed T3 Python scope passes Ruff check/format, py_compile, and diff checks. Repository-wide Ruff remains non-clean only in untouched legacy baseline files (10 import-order findings and 96 files requiring historical formatting), which this surgical PR does not rewrite.
- Latest replacement-CI checkpoint: GitHub run `29693960109` passed the complete Node job. Python finished with 2 failures and 2 errors across three portability/determinism categories: OpenCV golden-score precision, POSIX lease-handle assertions, and post-open path-disappearance normalization. The scoped repairs use fixed fixture bytes plus a contract-builder-only deterministic golden seam without changing production propagation, platform-neutral lease-release assertions, and stable `source_changed` mapping only after a successful open. The final repair diff passes 7 focused tests, Ruff check/format, and the current-diff official 2,263-test route; independent repair review is APPROVE with P1=0/P2=0. The required second replacement GitHub run is recorded in the merge checkpoint below.
- Latest remote-feedback checkpoint: final head `d461056fab8641f207f7d4016c46535de1d4b28d` completed its 2026-07-19T16:48:00Z–16:58:00Z window with zero issue comments and zero inline review threads; the only review body was Copilot's earlier non-actionable “PR exceeds 300 files” service-capacity note.
- Latest merge checkpoint: replacement run `29695366761` passed the Node job in 5m42s and Python job in 13m10s; the official Python verification step itself ran in 11m17s. The final 2026-07-19T16:58:40Z fetch found the exact expected head, clean merge state, zero issue comments, zero inline comments, and zero review threads. PR #114 merged at 2026-07-19T16:59:13Z as `8bd18841f707c283c2dd039284b024c3da99cc2d`; local and remote feature branches were removed while the user's dirty baseline was preserved.
- Latest operational-startup checkpoint: the stale standalone frontend on port 5173 had no backend and returned HTTP 500 for its source list. A direct current-`main` Uvicorn run completed the sealed real-data recovery after more than 45 seconds, became healthy, and served the Step 3 inputs/configs/runs/evidence APIs. The managed launcher reproduced a timeout only while Uvicorn was still in application startup. Its append-only log also contains an older missing-child-plan traceback whose source line numbers predate the current merged implementation; that historical exception is not the reproduced current failure.
- Latest PR-T3R validation checkpoint: the focused launcher suite passes 33/33; scoped Ruff check and format check pass; `git diff --check` passes. The repaired real `pnpm start` completed successfully against the same sealed data, `pnpm status` reported backend and frontend `RUNNING`, frontend health and source-list proxy calls returned HTTP 200, and the Production page no longer showed the source-list 500.
...
- Latest PR-T3H publication checkpoint: GitHub PR #124 opened MERGEABLE at product head `11fca3dc17c71b87b1e041620a39709fbfddcc40` against base `8d0fd8b93a18e1bb50fa2aac01ecf8717724f825`; Node and Python CI started at 2026-07-20T22:35Z. The following status-only commit records this publication and becomes the final head from which the complete 10-minute feedback window is measured.
- Latest PR-T3H merge checkpoint: final head `7dd5aedced05c5e93a368628e40bfe1be989c233` completed the full 2026-07-20T22:35:55Z–22:45:55Z remote-feedback window. Node passed at 22:40:53Z and Python passed at 22:48:52Z; the final audit found zero issue comments, zero inline comments, zero review threads, and Copilot reviewed 6/6 changed files with no comments. PR #124 merged MERGEABLE at 22:49:42Z as `6b4df63b944535ab2b62b75c1d7b79cb58732df1`; local and remote product branches were removed while the user's dirty baseline was preserved.
- Latest PR-T3H closeout checkpoint: docs-only PR #125 corrected its one valid Copilot comment, restarted and completed the full 2026-07-20T23:00:00Z–23:10:00Z feedback window, passed Node at 23:05:59Z and Python at 23:12:58Z, and ended with zero issue comments and no unresolved review thread. It merged at 23:13:31Z as `abcaf6eacb731629cb05b04e5347820cca8263c8`; the closeout branch was removed and the user's dirty baseline remained preserved.
...
- Latest PR-T3I local checkpoint: the bounded editor now exposes six keyboard-accessible list tabs, one human-labeled parameter per row, 44×44 localized help popovers, preserved drafts across tabs, responsive stacking, and unchanged backend values/paths. Authoritative V2 is the sole acceptance result; `retune_required` and missing-V2 states cannot expose acceptance, while legacy zero-tracklet continuity is explicitly non-applicable. The same-frame probe guide keeps exact 2–6 profile selection ahead of default-collapsed technical metadata and sends trustworthy ready/zero-candidate evidence into the existing manual-box annotation path without authorizing training. Full Web Vitest passes 1,273/1,273 with S/B/F/L 93.20/90.69/97.49/94.27; both Web TypeScript checks, production build/cutover verification, scoped Prettier, Ruff lint/format, the 159-test API service route with 2 skips and 25 subtests, and the targeted Chromium 100%-lost/mobile-overflow path pass. Live-browser inspection confirms tab selection, localized help, Escape focus restoration, truthful V2 blocking, zero Accept controls, exact-pending reconciliation, and visible selected-profile count. Final specification review is COMPLIANT and quality/security review is APPROVE with P0=0/P1=0/P2=0/P3=0.
- Latest PR-T3I baseline-gate checkpoint: the local official Python route reached all 2,266 tests and reported two timing failures plus 56 same-class cleanup errors after a Windows `execution.lock` remained held. Isolation reduced this to `test_cancellation_after_commit_start_is_rejected_and_commit_wins`, whose fixed five-second wait is exceeded by the current machine's roughly 4–6-second pre-publication work; a longer observation reached `committing` and then `ready`. The exact untouched `origin/main` commit `abcaf6eacb731629cb05b04e5347820cca8263c8` reproduces that same timeout, while its GitHub CI run `29786531115` is green. This is a pre-existing local timing baseline, not a PR-T3I product regression; PR-T3I's Python scope remains green and GitHub Node/Python CI is still required before merge.
- Latest PR-T3I publication checkpoint: GitHub PR #126 opened ready-for-review and MERGEABLE at product head `16c5d8010e734c8275d659ac8d9d43c4a1613a1c` against base `abcaf6eacb731629cb05b04e5347820cca8263c8`; Node and Python CI started at 2026-07-21T00:07:19Z in run `29789355740`. The following status-only commit records publication and becomes the final head from which the complete 10-minute feedback window is measured.
- Latest PR-T3I CI-repair checkpoint: replacement run `29789414085` passed generated-client and 1,273-test Web validation, then Chromium reported one valid failure: a pre-existing class-mapping scenario still located the renamed tuning control by obsolete technical path `detector.allowed_labels`. The locator now uses the exact accessible human label `Accepted football labels`; `exact: true` prevents matching the adjacent “Explain …” help button, while the test still verifies the submitted backend patch contains both unchanged raw label values. The exact scenario passes 1/1, the complete Chromium route passes 44/44, scoped Prettier and diff checks pass, and independent review is APPROVE with no findings. The corrective push below supersedes the earlier feedback window and starts a new complete 10-minute window.
- Latest PR-T3I merge checkpoint: exact final head `3c85cc48285e652fa368e935f82c62cfe15c0acb` passed replacement GitHub Actions run `29789949039`; Node completed successfully at 2026-07-21T00:25:17Z and Python at 00:31:15Z. The complete 2026-07-21T00:19:24Z–00:29:24Z remote-feedback window and final 00:31:38Z audit found zero issue comments, zero inline comments, zero review threads, and a Copilot 12/12 review with no comments; merge state was CLEAN and MERGEABLE. PR #126 merged at 00:32:03Z as `298bb3c6f5291f965a3a705381a3c1adbfeaa73b`; local and remote product branches were removed while the user's dirty baseline was preserved.
- Latest PR-T3I closeout-publication checkpoint: docs-only PR #127 opened ready-for-review and MERGEABLE at closeout head `ebfbd55e2c4a848ab73a17ef9e8c22fa1869fc4c` against base `298bb3c6f5291f965a3a705381a3c1adbfeaa73b` at 2026-07-21T00:37:15Z. Its scope is limited to this status file and the canonical Step 3 plan; the user's dirty 2026-07-14 plan and `pnpm-workspace.yaml` remain excluded. The following status-only commit records publication and becomes the final head from which the complete 10-minute feedback window is measured.
- Latest PR-T3I closeout-merge checkpoint: exact final head `980dedafaa35875d5f9637cdd193b660823594c7` passed GitHub Actions run `29790855527`; Node completed successfully at 2026-07-21T00:44:23Z and Python at 00:51:07Z. The complete 2026-07-21T00:38:05Z–00:48:05Z remote-feedback window and final 00:51:34Z audit found zero issue comments, zero inline comments, zero review threads, and a Copilot 2/2 review with no comments; merge state was MERGEABLE. PR #127 merged at 00:51:46Z as `3ac7048dec6075bcfd8be1407cb95e5d49881592`; local and remote closeout branches were removed while the user's dirty baseline was preserved.
...
- Blockers: no PR-T3I engineering blocker remains. The real-video `insufficient_evidence` decision remains a fail-closed program gate for PR-T4, which must not start training or 100–300-box expansion until a new disjoint frozen check satisfies support/interval rules and explicitly authorizes expansion.
- Next: deploy or restart the merged application, refresh Step 3, reconcile the preserved exact pending probe request, and create trustworthy ready evidence. If a ready probe returns zero suggestions, use the manual-box path to mark the ball; then complete a new layered development batch and disjoint frozen 20–50-frame check. Do not start PR-T4 unless that check explicitly passes and authorizes expansion.
...
| PR-T3H | `codex/tiny-ball-probe-lineage-compatibility` | completed · Node/Python CI passed · spec COMPLIANT · quality/security APPROVE · product branch cleaned · PR-T4 remains unauthorized | https://github.com/ferryhe/football_tracking/pull/124 | complete 2026-07-20T22:35:55Z–22:45:55Z · final audit 22:49Z · zero actionable feedback | `6b4df63b` · 2026-07-20T22:49:42Z |
| PR-T3I | `codex/tiny-ball-step3-operator-controls` | completed · replacement Node/Python CI passed · spec COMPLIANT · quality/security APPROVE · branches cleaned · PR-T4 remains unauthorized | https://github.com/ferryhe/football_tracking/pull/126 | complete 2026-07-21T00:19:24Z–00:29:24Z · final audit 00:31:38Z · zero actionable feedback | `298bb3c6` · 2026-07-21T00:32:03Z |
...
- Initial GitHub run `29693623734` exposed no product or assertion failure: only two intentionally exhaustive frontend security matrices exceeded Vitest's default five-second test limit on Linux after 1,235 assertions passed. A scoped correction gives only those two matrices a bounded 30-second limit; the focused pair and the full 1,237-test frontend route pass locally with unchanged coverage, and a replacement CI run was required before merge.
- Replacement GitHub run `29693960109` passed the complete Node job. Python finished with 2 failures and 2 errors across OpenCV golden-score precision, POSIX lease-handle assertions, and post-open path-disappearance normalization. The scoped repairs use fixed fixture bytes plus a contract-builder-only deterministic golden seam without changing production propagation, platform-neutral lease-release assertions, and stable `source_changed` mapping only after a successful open. The repair diff passes 7 focused tests, Ruff check/format, and the current-diff official Python route with 2,263 tests and 71 skips in 628.543 seconds; independent repair review is APPROVE with no P1/P2 findings. A second replacement GitHub run was required before merge.
...
- Define that locked-but-readable behavior for transient mutation locks (pending request, running trial, or save/rerun in flight). A sealed accepted trial retains the established explicit unlock-and-invalidate workflow; PR-T3I must not reload a potentially changed mutable base config and present it as the accepted run's immutable parameter truth.
