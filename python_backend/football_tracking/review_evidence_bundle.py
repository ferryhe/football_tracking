    ...
    *,
    ...
    try:
        ...
        target_root = _require_safe_relative_path(
            _required_mapping(_required_mapping(manifest.get("packages"), "packages").get("target_application"), "target_application").get("root"),
...
def validate_review_evidence_bundle(bundle_dir: Path) -> ValidatedReviewEvidenceBundle:
    ...
    root_contract_sha256 = _required_sha256(
        target.get("root_contract_sha256"), "target.root_contract_sha256"
    )
    ...
    if _positive_int(
        target.get("max_manual_review_windows"), "target.max_manual_review_windows"
    ) != max_windows:
...
def activate_review_evidence_bundle(
    ...
    *,
    ...
    if (
        ...
        and bundle.bundle_sha256 != _required_sha256(
    ...
    expected = {
        ...
        "root_contract_sha256": _required_sha256(
            expected_root_contract_sha256, "expected_root_contract_sha256"
        ),
    }
...
def _require_inventory_path(
    inventory: Mapping[str, Mapping[str, Any]], relative: PurePosixPath, label: str
) -> Path:
...
def _dataset_population(path: Path, label: str) -> dict[str, set[str]]:
    ...
    result = {name: set() for name in ("candidate_id", "variant_id", "video_sha256", "group_id", "split_group", "temporal_group")}
