...
def inspect_terminal_tail_review(output_dir: Path) -> dict[str, Any]:
    ...
    action_signal_report_sha256 = _required_sha256(
        action_descriptor.get("sha256"), "action-signal report sha256"
    )
    ...
    if raw_audit is None:
        ...
        if (
            ...
            and any(
                isinstance(item, dict) and item.get("code") == TERMINAL_TAIL_LIMITATION
    ...
    boundary_event = (
        ...
        and len(boundary_events) == 1
        and isinstance(boundary_events[0], dict)
        else None
    )
    ...
    if (
        ...
        or _required_sha256(contract_source.get("video_sha256"), "tracking source video sha256")
        != source_video_sha256
    ...
    if (
        ...
        != binding_sha256
        or _required_mapping(acknowledgement.get("evidence"), "terminal-tail acknowledgement evidence")
        != evidence
    ):
